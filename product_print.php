            ...
            <div class="container-xxl flex-grow-1 container-p-y">
              ...
              <div class="card">
               <div 
                      ...
                      <h5 class="card-title mb-sm-0 me-2" >PRINT LIST</h5>
               ...
               &nbsp;   <button class="btn btn-secondary add-new btn-primary" tabindex="0" onclick="addPrint();" aria-controls="DataTables_Table_0"
                 type="button" data-bs-toggle="offcanvas" data-bs-target="#offcanvasAddUser"><span>
                  <i class="ti ti-plus me-0 me-sm-1 ti-xs"></i>
                <span class="d-none d-sm-inline-block">Add New Print</span></span></button>
                      </div>
                    </div>
                ...
                <div class="card-body">
                  ...
                  ?>
                     <input type="text" hidden name="id[]" id="id<?php echo $sno; ?>" value="<?php echo $rows['id'];?>">
                      ...
                      <td><?php echo $rows['product_print'];?></td>
                                              
                          <td nowrap>
                          <button type="button" data-bs-toggle="offcanvas" data-bs-target="#offcanvasAddUser"
                          class="btn btn-outline-primary"
                          id="edit<?php echo $sno;?>" onclick="getPrint(edit<?php echo $sno;?>.id);" 
                         >
                          ...
                          id="del<?php echo $sno;?>" onclick="delPrint(del<?php echo $sno;?>.id);" >
               ...
               <tr><td colspan="5" align="center">  <p>No Data Found</p></td> </tr>
                ...
                <div
                  ...
                  <div class="offcanvas-header">
                    <h5 id="offcanvasAddUserLabel" class="offcanvas-title"><span id="form-title">Add</span> Print</h5>
                    <button
                      type="button"
                  ...
                  <div class="offcanvas-body mx-0 flex-grow-0 pt-0 h-100">
                    <form action="" method="post">
                      ...
                      <div class="mb-3">
                      <label class="form-label" for="add-user-fullname">Product Print</label>
                        <input
                          type="text"
                          class="form-control"
                          id="product_print"
...
if(isset($_POST['submit']))
{
  ...
  if($id==""){
   ...
   $sql="insert into product_print (product_print) values('$product_print')"; 
   $result1=mysqli_query($conn, $sql);
  }
  else{
   echo $sql="UPDATE product_print SET product_print='$product_print' WHERE id='$id'"; 
    $result2=mysqli_query($conn, $sql);
  
  }
  if($result1) { 
    echo "<script>alert('Print Added Successfully');window.location='product_print.php';</script>";
  }
  else if($result2) { 
    echo "<script>alert('Print Updated Successfully');window.location='product_print.php';</script>";
...
if(data.sts == 'ok'){
                    ...
                    $('#id').val(data.id);  
                    $('#product_print').val(data.product_print);  
                 
}

      }
    };
    xmlhttp.open("GET","ajax/getPrint.php?id="+user_id,true);
    xmlhttp.send();
  }
}
</script>
...
if(data.sts == 'ok'){
  ...
  window.location='product_print.php';

                   
}
      }
    };
    xmlhttp.open("GET","ajax/delPrint.php?id="+user_id,true);
    xmlhttp.send();
  }
}
}

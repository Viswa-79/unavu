<?php include "config.php"; ?>
  ...
  <body>
   ...
   if($rtype=='Cutting'){
    $rtype="Cutpanel";
    $table = "cutpanel_outward";
    $tables = "cutpanel_outward1";
   }
 else if($rtype=='Printing'){
    $table = "printing_outward";
    $tables = "printing_outward1";
   }
  else if($rtype=='Packing'){
    $table = "packing_outward";
    $tables = "packing_outward1";
   }
  else if($rtype=='Sewing'){
    $table = "sewing_outward";
    $tables = "sewing_outward1";
   }
  else if($rtype=='Checking'){
    $table = "checking_outward";
    $tables = "checking_outward1";
   }
    $table;
    $tables;
 ?>
...
?> </h5>
                ...
                <tbody>
                 ...
                 $sql4 = " SELECT * FROM $table s left join $tables s1 on s.id=s1.cid left join partymaster m on s.partyname=m.id left join item_master a on s1.itemno=a.id left join color n on s1.color=n.id where date between '$fromdate' and '$todate' $rr order by date asc";
             ...
             <?php $sno++;
               $pcs1+=number_format((float)$wz1['pcs'], 2, '.', '');
               }
             ?>
...
<a href="outward_report.php" class="btn btn-primary mt-4"><i class="ti ti-arrow-left me-sm-1 me-0"></i>Back</a>&nbsp;&nbsp;

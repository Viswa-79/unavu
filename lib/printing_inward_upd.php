        ...
        <div class="layout-page">
		 ...
		 ?>               
                      ...
                      <form class="card-body" action="" method="post" enctype="multipart/form-data" autocomplete="off">
                        ...
                        <!-- Personal Info -->
                        <?php
                              $sql4 = " SELECT * FROM printing_inward WHERE id='$sid'";
                    ...
                    <?php } ?>
                          ...
                          <!-- <div class="col-md-2">
                            ...
                            <select name="process" id="process" class="select form-select" >
                                 <option value="">Select</option>
                                  <?php 
                           $sql = "SELECT * FROM process where process IN ('Printing','Sewing') order by process asc";
                           $result = mysqli_query($conn, $sql);
                            while($rw1 = mysqli_fetch_array($result))
                           { ?>
                       ...
                       <?php } ?>
                        ...
                        <table class="table table-border border-top table-hover">
                              ...
                              <?php
                                  $sno=1; $i=0;
                              $sql4 = " SELECT * FROM printing_inward1 Where cid='$sid' order by id asc";
                              $result4 = mysqli_query($conn, $sql4);
              ...
              <select name="itemno[]" id="itemno<?php echo $i;?>"  class="select form-select" >
                                <option value="">Select</option>
							<?php 
					$sql1 = "SELECT * FROM order2 m left join order1 s on s.id=m.cid join item_master n on m.itemno=n.id where s.ord_no='$ordno' order by itemno asc";
              ...
              <select name="quality[]" style="width:300px" id="quality<?php echo $i;?>"  class="select form-select" >
                                <option value="">Select</option>
							<?php 
					$sql2 = "SELECT * FROM costing4 m left join costing s on s.id=m.cid join quality_master n on m.quality=n.id where s.orderno='$ordno' ";
...
if (isset($_POST['submit'])) {
  ...
  $sql = "UPDATE printing_inward SET dcno='$dcno',refno='$refno',date='$date',partyname='$partyname',process='$process',vehicleno='$vehicleno',
 dvrname='$dvrname' WHERE id='$cid'";
  ...
  foreach ($_POST['pcs'] as $key => $val) {
    ...
    $sql1 = "UPDATE  printing_inward1 SET cid='$cid',ref='$ref',orderno='$orderno',itemno='$itemno',quality='$quality',color='$color',pcs='$pcs',descr='$descr' WHERE id='$rid'";
    $result1 = mysqli_query($conn, $sql1);
  ...
  echo "<script>alert('Printing Updated Successfully');window.location='printinlist.php';</script>";

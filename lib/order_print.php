...
$sql1 = " SELECT *,e.id as id,e.print as prints,e.quality as quality FROM order2 e left join item_master p on e.itemno=p.id WHERE e.cid='$sid'";
$result1 =mysqli_query($conn, $sql1);
while($b3=mysqli_fetch_object($result1))
  ...
  <tr style="border-bottom:1px solid black;" >
	...
	{ ?> 
	 ...
	 <?php if($b3->quality!=''){ ?>   <strong>Quality:</strong><?php echo $b3->quality;?></br> <?php } ?>
	  
	  
 <?php
  if($b3->size!='' && $b3->size!='-'){ ?>     <strong>Size:</strong><?php echo $b3->size; }?>
      </div></td>
      <td  style="border-right:1px solid black;" ><div align="center" style="margin-left:2%;"><?php echo $b3->price;?> / <?php if($b3->unit!='Set'){echo substr($b3->unit, 0, -1); }else { echo $b3->unit; }?>
...
<td style="border:none; border-right:none; vertical-align:text-top;" >:</td>
 ...
 $sno=1;
$sql1 = " SELECT * FROM order2  WHERE cid='$sid' group by pack";
$result1 =mysqli_query($conn, $sql1);
while($b3=mysqli_fetch_array($result1))
{
echo $sno++;echo ". ";echo $pack=$b3['pack'];echo " ( ";

$sql2 = "SELECT *,i.code as code FROM order2 o2 left join item_master i on o2.itemno=i.id WHERE cid='$sid' and o2.pack='$pack'";
$result2 =mysqli_query($conn, $sql2);
while($b4=mysqli_fetch_array($result2))
{
	$code1=explode("-",$b4['code']);
	$code[]=$code1[0];
}
echo $itemnos=implode(", ",$code);echo " ) ";echo "<br>";
unset($code);
}
	?></span>
</td>

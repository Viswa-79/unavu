...
To: <?php echo date('d/m/Y',strtotime($_REQUEST['todate']));?></h5>
         ...
         $rr='';  
                 ...
                 $sql4 = " SELECT *,s.id as idn FROM salesinvoice s left join partymaster p on s.clearingagent=p.id where date between '$fromdate' and '$todate' $rr order by invoiceno asc";
                 $result4 = mysqli_query($conn, $sql4);
                 while($wz1 = mysqli_fetch_array($result4)){
                    $date1=date('d/m/Y',strtotime($wz1['date']));
                    $date2=date('d/m/Y',strtotime($wz1['sbdate']));
                    $id =$wz1['idn'];
                    if($wz1['dstatus']==0)
                    $dstatus="Pending";
                elseif($wz1['dstatus']==1)
                    $dstatus="Received";
                else
                    $dstatus="";
               ...
               <td nowrap><a href="sales_invoice_upd.php?cid=<?php echo base64_encode($id);?>" target="blank" ><?php echo $wz1['invoiceno']; ?></a></td>      
               <td><?php echo $date1; ?></td>      
               <td><?php echo $wz1['sbno']; ?></td>      
               <td><?php echo $date2; ?></td>      
               <td style="text-align:right"><?php echo $wz1['port']; ?></td>      
               <td><?php echo $wz1['fobvalue']; ?></td>      
               <td><?php echo $wz1['dbkvalue']; ?></td>      
               <td><?php echo $dstatus; ?></td>      
               <td><?php echo $wz1['scroll']; ?></td>      
...
<a href="drawback.php" class="btn btn-primary mt-4"><i class="ti ti-arrow-left me-sm-1 me-0"></i>Back</a>&nbsp;&nbsp;

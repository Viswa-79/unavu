...
<title>Fabric Process Print</title>
<style type="text/css">
.style1
{
color:#990000;
...
$sid=base64_decode($_GET['cid']);
  ...
  $sql = " SELECT *,e.id as id FROM fabric_process e left join staff p on e.follow=p.id WHERE e.id='$sid'";
...
<body>
    ...
    <div id="mydiv">
   <?php
      $a11=mysqli_query($conn,"SELECT * FROM fabric_process1 where cid='$sid'");
  ...
  <table width="800"  align="center" style="border-collapse:collapse; border:none;">
	...
	<strong> <?php echo $so['checking'];?></strong>	 </td>
	  ...
	  <br />
<b>FABRIC PROCESS PLANNING</BR></BR>
...
$i=1;
$sql2 = " SELECT *,e.id as id,e.print as print,e.size as size FROM fabric_process1 e left join item_master p on e.itemno=p.id  WHERE cid='$sid'";
...
while($b3=mysqli_fetch_object($result2))
  ...
  <tr style="border-bottom:1px solid black;" >
      ...
      <strong><?php if($b3->visible!='0'){ ?>Item no:<?php echo $b3->code;?>/ &nbsp;<?php } ?>PO No:<?php echo $b3->pono;?></strong></br><strong>Description :</strong><?php echo $b3->itemdesc;?></br>
	<?php if($b3->print!='')
	{ ?>  
...
<table  width="800" align="center" style=" border-top:1px solid #000000; border-collapse: collapse;">
 ...
 <?php
 $sql = " SELECT *,e.id as id FROM fabric_process e left join staff p on e.prepared=p.id WHERE e.id='$sid'";
                 $result =mysqli_query($conn, $sql);
                 $so=mysqli_fetch_array($result);
                 ?>
   <tr>

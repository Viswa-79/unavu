...
<?php
$fg1="select max(dcno) as id from packing_inward";
		   //echo $fg1;
...
if (isset($_POST['submit'])) {
  ...
  $dvrname = $_POST['dvrname'];
    ...
    $sql = "INSERT into packing_inward (dcno,refno,date,partyname,vehicleno,dvrname) values ('$dcno','$refno','$date','$partyname','$vehicleno','$dvrname')";
    $result = mysqli_query($conn, $sql);
  ...
  foreach ($_POST['pcs'] as $key => $val) {
    ...
    if ($orderno != '') {
       $sql1 = "INSERT into packing_inward1 (cid,orderno,ref,itemno,pcs,descr) 
 values ('$cid','$orderno','$ref','$itemno','$pcs','$descr')";
  ...
  echo "<script>alert('Packing Inward Registered Successfully');window.location='packing_inward.php';</script>";
...
if(sts==1)
{
  document.getElementById('packdcno').innerHTML =r1[1];
					
            }
						 
            //   if(sts==2) 
            // {
            //   alert("Packing Inward Already Made For This Dc No.!");
            //   document.getElementById('refno').value='';
            //   document.getElementById('packdcno').innerHTML ='';
            // }
            // else if(sts==2)
            // {
            //   alert("Order No. Not Available.!");
    ...
    xmlhttp.open("GET","ajax/get_packdcno.php?refno="+value+"&q="+form,true);
    xmlhttp.send();
  }
}
</script>
...
$('#partyname').val(data.partyname);
    ...
    xmlhttp.open("GET","ajax/getpackdetails.php?id="+value,true);
    xmlhttp.send();
  }
}
</script>